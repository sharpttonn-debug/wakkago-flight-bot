...
// Live endpoint target matching your active dashboard instance CLI string layout
const WASENDER_API_URL = 'https://wasenderapi.com'; 
const MY_SERVER_URL = 'https://watravel-bot.onrender.com';

// 1. Core HTTP Browser Landing Route Verification
app.get('/', (req, res) => {
    res.status(200).send(`🚀 Fresh Wakkago Production Engine is Online and Operational at ${MY_SERVER_URL}!`);
});
...
app.post('/webhook/whatsapp', async (req, res) => {
    try {
        ...
        // Unpack explicit WASender structure layers safely
        const incomingData = req.body?.data?.messages || req.body?.data || req.body;
        const msgObject = Array.isArray(incomingData) ? incomingData[0] : incomingData;
        
        // Extract sender parameters using explicit documented properties
        let rawPhone = msgObject?.key?.remoteJid || 
                       msgObject?.key?.cleanedSenderPn ||
                       msgObject?.from || 
                       msgObject?.phone || "";
        ...
        let incomingText = msgObject?.messageBody || 
                           ...
                           msgObject?.message?.extendedTextMessage?.text ||
                           msgObject?.text || "";

        if (!rawPhone || !incomingText) {
            console.log('🛑 Aborting: Unable to isolate clean recipient phone or text parameters.');
            return;
        }
        ...
        if (incomingText.toLowerCase().trim() === 'test') {
            console.log('🎯 Diagnostic test command matched! Dispatching immediate echo response...');
            await sendWhatsAppMessage(senderPhone, 'Hello! Your brand new watravel-bot server is 100% active. 🚀');
            return;
        }
